// LS-RecentPlays
var RPlists=[];function saveRPlist(){localStorage.setItem("RPlaylist",JSON.stringify(RPlists))}
function loadRPlist(){var value=localStorage.getItem("RPlaylist");if(value!==null){RPlists=JSON.parse(value)}}
function pushToRPlist(value,cover_url,track_name,artist_name,track_url){var obj={RPcoverart:cover_url,RPtitle:track_name,RPartist:artist_name,RTurl:track_url,MusicID:value,};RPlists.push(obj)}
// Add Tracklist Top
...
var Turl = $('#getlink').val();
  function playPause() {
    if (isPlaying()) {
	  ...
	  localStorage.setItem("Rplayed-Title", tracklist[i].song);
      localStorage.setItem("Rplayed-Artist", tracklist[i].artist);
      localStorage.setItem("Rplayed-Coverurl", tracklist[i].artwork);
      localStorage.setItem("Rplayed-Url", Turl);
      var Mid = $(".music_id").text();
      var newRPlist = Mid;
      var cover_url = tracklist[i].artwork;
      var track_name = tracklist[i].song;
      var artist_name = tracklist[i].artist;
      var track_url = Turl;
      pushToRPlist(newRPlist, cover_url, track_name, artist_name, track_url);
      saveRPlist();
    }
  }
